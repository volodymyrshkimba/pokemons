...
`);};var DEFAULT_MAX_BUFFER=1e3*1e3*100,getEnv=({env:envOption,extendEnv,preferLocal,localDir,execPath})=>{let env=extendEnv?{...process6__default.default.env,...envOption}:envOption;return preferLocal?npmRunPathEnv({env,cwd:localDir,execPath}):env},handleArguments=(file,args,options={})=>{let parsed=import_cross_spawn.default._parse(file,args,options);return file=parsed.command,args=parsed.args,options=parsed.options,options={maxBuffer:DEFAULT_MAX_BUFFER,buffer:!0,stripFinalNewline:!0,extendEnv:!0,preferLocal:!1,localDir:options.cwd||process6__default.default.cwd(),execPath:process6__default.default.execPath,encoding:"utf8",reject:!0,cleanup:!0,all:!1,windowsHide:!0,verbose:verboseDefault,...options},options.env=getEnv(options),options.stdio=normalizeStdio(options),process6__default.default.platform==="win32"&&path2__default.default.basename(file,".exe")==="cmd"&&args.unshift("/q"),{file,args,options,parsed}},handleOutput=(options,value,error)=>typeof value!="string"&&!buffer.Buffer.isBuffer(value)?error===void 0?void 0:"":options.stripFinalNewline?stripFinalNewline(value):value;function execa(file,args,options){let parsed=handleArguments(file,args,options),command=joinCommand(file,args),escapedCommand=getEscapedCommand(file,args);logCommand(escapedCommand,parsed.options),validateTimeout(parsed.options);let spawned;try{spawned=childProcess__default.default.spawn(parsed.file,parsed.args,parsed.options);}catch(error){let dummySpawned=new childProcess__default.default.ChildProcess,errorPromise=Promise.reject(makeError({error,stdout:"",stderr:"",all:"",command,escapedCommand,parsed,timedOut:!1,isCanceled:!1,killed:!1}));return mergePromise(dummySpawned,errorPromise),dummySpawned}let spawnedPromise=getSpawnedPromise(spawned),timedPromise=setupTimeout(spawned,parsed.options,spawnedPromise),processDone=setExitHandler(spawned,parsed.options,timedPromise),context={isCanceled:!1};spawned.kill=spawnedKill.bind(null,spawned.kill.bind(spawned)),spawned.cancel=spawnedCancel.bind(null,spawned,context);let handlePromiseOnce=onetime_default(async()=>{let[{error,exitCode,signal,timedOut},stdoutResult,stderrResult,allResult]=await getSpawnedResult(spawned,parsed.options,processDone),stdout=handleOutput(parsed.options,stdoutResult),stderr=handleOutput(parsed.options,stderrResult),all=handleOutput(parsed.options,allResult);if(error||exitCode!==0||signal!==null){let returnedError=makeError({error,exitCode,signal,stdout,stderr,all,command,escapedCommand,parsed,timedOut,isCanceled:(parsed.options.signal?parsed.options.signal.aborted:!1),killed:spawned.killed});if(!parsed.options.reject)return returnedError;throw returnedError}return {command,escapedCommand,exitCode:0,stdout,stderr,all,failed:!1,timedOut:!1,isCanceled:!1,killed:!1}});return handleInput(spawned,parsed.options),spawned.all=makeAllStream(spawned,parsed.options),addPipeMethods(spawned),mergePromise(spawned,handlePromiseOnce),spawned}function execaNode(scriptPath,args,options={}){args&&!Array.isArray(args)&&typeof args=="object"&&(options=args,args=[]);let stdio=normalizeStdioNode(options),defaultExecArgv=process6__default.default.execArgv.filter(arg=>!arg.startsWith("--inspect")),{nodePath=process6__default.default.execPath,nodeOptions=defaultExecArgv}=options;return execa(nodePath,[...nodeOptions,scriptPath,...Array.isArray(args)?args:[]],{...options,stdin:void 0,stdout:void 0,stderr:void 0,stdio,shell:!1})}var MAX_START_TIME=3e4,vitestModulePath=join(__dirname,"node","vitest.mjs"),eventQueue=[],child,ready=!1,bootTestRunner=async channel=>{let stderr=[];function reportFatalError(e){channel.emit(coreEvents.TESTING_MODULE_CRASH_REPORT,{providerId:TEST_PROVIDER_ID,error:{message:String(e)}});}let forwardRun=(...args)=>child?.send({args,from:"server",type:coreEvents.TESTING_MODULE_RUN_REQUEST}),forwardCancel=(...args)=>child?.send({args,from:"server",type:coreEvents.TESTING_MODULE_CANCEL_TEST_RUN_REQUEST}),forwardStore=(...args)=>{child?.send({args,from:"server",type:STORE_CHANNEL_EVENT_NAME});},killChild=()=>{channel.off(coreEvents.TESTING_MODULE_RUN_REQUEST,forwardRun),channel.off(coreEvents.TESTING_MODULE_CANCEL_TEST_RUN_REQUEST,forwardCancel),channel.off(STORE_CHANNEL_EVENT_NAME,forwardStore),child?.kill(),child=null;},exit=(code=0)=>{killChild(),eventQueue.length=0,process.exit(code);};process.on("exit",exit),process.on("SIGINT",()=>exit(0)),process.on("SIGTERM",()=>exit(0));let startChildProcess=()=>new Promise((resolve2,reject)=>{child=execaNode(vitestModulePath),stderr=[],child.stdout?.on("data",log),child.stderr?.on("data",data=>{data.toString().match(/^\u001B\[33m/)||(log(data),stderr.push(data.toString()));}),channel.on(STORE_CHANNEL_EVENT_NAME,forwardStore),child.on("message",result=>{if(result.type==="ready"){for(;eventQueue.length;){let{type,args}=eventQueue.shift();child?.send({type,args,from:"server"});}channel.on(coreEvents.TESTING_MODULE_RUN_REQUEST,forwardRun),channel.on(coreEvents.TESTING_MODULE_CANCEL_TEST_RUN_REQUEST,forwardCancel),resolve2();}else if(result.type==="error"){killChild(),log(result.message),log(result.error);let error=new Error(`${result.message}
${result.error}`);ready?reportFatalError(error):reject(error);}else channel.emit(result.type,...result.args);});}),timeout=new Promise((_,reject)=>setTimeout(reject,MAX_START_TIME,new Error(`Aborting test runner process because it took longer than ${MAX_START_TIME/1e3} seconds to start.`)));await Promise.race([startChildProcess(),timeout]).catch(e=>{throw reportFatalError(e),eventQueue.length=0,e});},runTestRunner=async(channel,initEvent,initArgs)=>{!ready&&initEvent&&eventQueue.push({type:initEvent,args:initArgs}),child||(ready=!1,await bootTestRunner(channel),ready=!0);};var checkActionsLoaded=configDir=>{common.checkAddonOrder({before:{name:"@storybook/addon-actions",inEssentials:!0},after:{name:"@storybook/experimental-addon-test",inEssentials:!1},configFile:isAbsolute(configDir)?join(configDir,"main"):join(process.cwd(),configDir,"main"),getConfig:configFile=>common.serverRequire(configFile)});},experimental_serverChannel=async(channel,options)=>{let core=await options.presets.apply("core"),builderName=typeof core?.builder=="string"?core.builder:core?.builder?.name,framework=await common.getFrameworkName(options),store=coreServer.experimental_UniversalStore.create({...storeOptions,leader:!0});if(!builderName?.includes("vite"))return framework.includes("nextjs")&&log(tsDedent.dedent`
      ...
      `),channel;let execute=eventName=>(...args)=>{args[0]?.providerId===TEST_PROVIDER_ID&&runTestRunner(channel,eventName,args);};if(channel.on(coreEvents.TESTING_MODULE_RUN_REQUEST,execute(coreEvents.TESTING_MODULE_RUN_REQUEST)),store.onStateChange(state=>{state.watching&&runTestRunner(channel);}),!core.disableTelemetry){let packageJsonPath=__require.resolve("@storybook/experimental-addon-test/package.json"),{version:addonVersion}=JSON.parse(fs.readFileSync(packageJsonPath,{encoding:"utf-8"}));channel.on(STORYBOOK_ADDON_TEST_CHANNEL,event=>{telemetry.telemetry("addon-test",{...event,payload:{...event.payload,storyId:telemetry.oneWayHash(event.payload.storyId)},addonVersion});}),store.onStateChange(async(state,previous)=>{state.watching&&!previous.watching&&await telemetry.telemetry("testing-module-watch-mode",{provider:TEST_PROVIDER_ID,watchMode:state.watching});}),channel.on(coreEvents.TESTING_MODULE_PROGRESS_REPORT,async payload=>{if(payload.providerId!==TEST_PROVIDER_ID)return;let status="status"in payload?payload.status:void 0,progress="progress"in payload?payload.progress:void 0,error="error"in payload?payload.error:void 0,config=store.getState().config;(status==="success"||status==="cancelled")&&progress?.finishedAt&&await telemetry.telemetry("testing-module-completed-report",{provider:TEST_PROVIDER_ID,status,config,duration:progress?.finishedAt-progress?.startedAt,numTotalTests:progress?.numTotalTests,numFailedTests:progress?.numFailedTests,numPassedTests:progress?.numPassedTests,numSelectedStories:payload.details?.selectedStoryCount??0}),status==="failed"&&await telemetry.telemetry("testing-module-completed-report",{provider:TEST_PROVIDER_ID,status,config,...options.enableCrashReports&&{error:error&&telemetry.sanitizeError(error)},numSelectedStories:payload.details?.selectedStoryCount??0});}),channel.on(coreEvents.TESTING_MODULE_CRASH_REPORT,async payload=>{payload.providerId===TEST_PROVIDER_ID&&await telemetry.telemetry("testing-module-crash-report",{provider:payload.providerId,...options.enableCrashReports&&{error:telemetry.cleanPaths(payload.error.message)}});});}return channel},previewAnnotations=async(entry=[],options)=>(checkActionsLoaded(options.configDir),entry),managerEntries=async(entry=[],options)=>{if(await options.presets.apply("ADDON_INTERACTIONS_IN_USE",!1)){let error=new Error(tsDedent.dedent`
      ...
      `);throw error.name="AddonConflictError",error}return entry},staticDirs=async(values=[],options)=>{if(options.configType==="PRODUCTION")return values;let coverageDirectory=common.resolvePathInStorybookCache(COVERAGE_DIRECTORY);return await promises.mkdir(coverageDirectory,{recursive:!0}),[{from:coverageDirectory,to:"/coverage"},...values]};

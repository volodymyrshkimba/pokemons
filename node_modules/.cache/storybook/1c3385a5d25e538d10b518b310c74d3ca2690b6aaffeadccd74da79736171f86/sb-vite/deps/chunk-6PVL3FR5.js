...
function getDocumentWidthAndHeight() {
  let container = import_global.global.document.documentElement, height = Math.max(container.scrollHeight, container.offsetHeight);
  return { width: Math.max(container.scrollWidth, container.offsetWidth), height };
}
function createCanvas() {
  let canvas = import_global.global.document.createElement("canvas");
  canvas.id = "storybook-addon-measure";
  let context = canvas.getContext("2d");
  invariant(context != null);
  let { width, height } = getDocumentWidthAndHeight();
  return setCanvasWidthAndHeight(canvas, context, { width, height }), canvas.style.position = "absolute", canvas.style.left = "0", canvas.style.top = "0", canvas.style.zIndex = "2147483647", canvas.style.pointerEvents = "none", import_global.global.document.body.appendChild(canvas), { canvas, context, width, height };
}
function setCanvasWidthAndHeight(canvas, context, { width, height }) {
  canvas.style.width = `${width}px`, canvas.style.height = `${height}px`;
  let scale = import_global.global.window.devicePixelRatio;
...
function rescale() {
  invariant(state.canvas, "Canvas should exist in the state."), invariant(state.context, "Context should exist in the state."), setCanvasWidthAndHeight(state.canvas, state.context, { width: 0, height: 0 });
  let { width, height } = getDocumentWidthAndHeight();
...
var colors2 = { margin: "#f6b26ba8", border: "#ffe599a8", padding: "#93c47d8c", content: "#6fa8dca8" };
...
function floatingAlignment(extremities) {
  let windowExtremities = { top: import_global.global.window.scrollY, bottom: import_global.global.window.scrollY + import_global.global.window.innerHeight, left: import_global.global.window.scrollX, right: import_global.global.window.scrollX + import_global.global.window.innerWidth }, distances = { top: Math.abs(windowExtremities.top - extremities.top), bottom: Math.abs(windowExtremities.bottom - extremities.bottom), left: Math.abs(windowExtremities.left - extremities.left), right: Math.abs(windowExtremities.right - extremities.right) };
  return { x: distances.left > distances.right ? "left" : "right", y: distances.top > distances.bottom ? "top" : "bottom" };
}
function measureElement(element) {
  let style = import_global.global.getComputedStyle(element), { top, left, right, bottom, width, height } = element.getBoundingClientRect(), { marginTop, marginBottom, marginLeft, marginRight, paddingTop, paddingBottom, paddingLeft, paddingRight, borderBottomWidth, borderTopWidth, borderLeftWidth, borderRightWidth } = style;
  top = top + import_global.global.window.scrollY, left = left + import_global.global.window.scrollX, bottom = bottom + import_global.global.window.scrollY, right = right + import_global.global.window.scrollX;
...
var deepElementFromPoint = (x, y) => {
  let element = import_global.global.document.elementFromPoint(x, y), crawlShadows = (node) => {
...
//# sourceMappingURL=chunk-6PVL3FR5.js.map

...
import "./chunk-5RLVS3NC.js";
import {
  vr
} from "./chunk-G3LRZGCS.js";
import "./chunk-MX2IDHH2.js";
import "./chunk-OGSAKKRA.js";
...
var Wrapper = vr.div({ position: "relative", maxWidth: 250, '&[aria-readonly="true"]': { opacity: 0.5 } });
var PickerTooltip = vr(qB)({ position: "absolute", zIndex: 1, top: 4, left: 4, "[aria-readonly=true] &": { cursor: "not-allowed" } });
var TooltipContent = vr.div({ width: 200, margin: 5, ".react-colorful__saturation": { borderRadius: "4px 4px 0 0" }, ".react-colorful__hue": { boxShadow: "inset 0 0 0 1px rgb(0 0 0 / 5%)" }, ".react-colorful__last-control": { borderRadius: "0 0 4px 4px" } });
var Note = vr(nI)(({ theme }) => ({ fontFamily: theme.typography.fonts.base }));
var Swatches = vr.div({ display: "grid", gridTemplateColumns: "repeat(9, 16px)", gap: 6, padding: 3, marginTop: 5, width: 200 });
var SwatchColor = vr.div(({ theme, active }) => ({ width: 16, height: 16, boxShadow: active ? `${theme.appBorderColor} 0 0 0 1px inset, ${theme.textMutedColor}50 0 0 0 4px` : `${theme.appBorderColor} 0 0 0 1px inset`, borderRadius: theme.appBorderRadius }));
...
var Input = vr(YT.Input)(({ theme, readOnly }) => ({ width: "100%", paddingLeft: 30, paddingRight: 30, boxSizing: "border-box", fontFamily: theme.typography.fonts.base }));
var ToggleIcon = vr(MarkupIcon)(({ theme }) => ({ position: "absolute", zIndex: 1, top: 6, right: 7, width: 20, height: 20, padding: 4, boxSizing: "border-box", cursor: "pointer", color: theme.input.color }));
...
var ColorControl = ({ name, value: initialValue, onChange, onFocus, onBlur, presetColors, startOpen = false, argType }) => {
  ...
  return import_react.default.createElement(Wrapper, { "aria-readonly": readonly }, import_react.default.createElement(PickerTooltip, { startOpen, trigger: readonly ? [null] : void 0, closeOnOutsideClick: true, onVisibleChange: () => addPreset(color), tooltip: import_react.default.createElement(TooltipContent, null, import_react.default.createElement(Picker, { color: realValue === "transparent" ? "#000000" : realValue, onChange: updateValue, onFocus, onBlur }), presets.length > 0 && import_react.default.createElement(Swatches, null, presets.map((preset, index) => import_react.default.createElement(qB, { key: `${preset.value}-${index}`, hasChrome: false, tooltip: import_react.default.createElement(Note, { note: preset.keyword || preset.value }) }, import_react.default.createElement(Swatch, { value: preset[colorSpace], active: color && id(preset[colorSpace]) === id(color[colorSpace]), onClick: () => updateValue(preset.value) }))))) }, import_react.default.createElement(Swatch, { value: realValue, style: { margin: 4 } })), import_react.default.createElement(Input, { id: getControlId(name), value, onChange: (e2) => updateValue(e2.target.value), onFocus: (e2) => e2.target.select(), readOnly: readonly, placeholder: "Choose color..." }), value ? import_react.default.createElement(ToggleIcon, { onClick: cycleColorSpace }) : null);
};
...
//# sourceMappingURL=Color-YHDXOIA2-QSWG22L3.js.map
